...
ZygorGuidesViewer:RegisterInclude("Arakkoa_Outcasts",[[
	...
	step
		...
		Click here to kill level 100 mobs for reputation |confirm
	step
	label "farm"
...
ZygorGuidesViewer:RegisterInclude("Steamwheedle_Preservation_Society",[[
	...
	step
	label "farm"
		kill Direfang Prowler##86931+, Warsong Wolf Trainer##87231+, Marsh Anaconda##87039+, Highmaul Guard##87221+
...
ZygorGuidesViewer:RegisterInclude("wod_jenkins",[[
	step
		...
		|confirm |only if not achieved(9058)
	step
		...
		This will start a timer, after the timer runs out he will run further into the dungeon, all the way to the Furnace
		If the enemies and 2 bosses are cleared before he does this, he will successfully retrieve his Devout shoulders
		|achieve 9058
	step
...
ZygorGuidesViewer:RegisterInclude("wod_talonkingqueen",[[
	step
		...
		_Revered_ |only if rep('Arakkoa Outcasts')==Revered
		_Exalted_ |only if rep('Arakkoa Outcasts')==Exalted
		|confirm |only if not achieved(9072)
	step
	label "menu"
		Completing quests in the Spires of Arak and Talador will give you rep with this faction
